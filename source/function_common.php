...
function verifyLogin($username = 0, $password = 0, $returnArr = false) {
    global $_SGLOBAL, $cookiepre, $authkey;
    $db = $_SGLOBAL['db'];
    require_once(S_ROOT . './models/mysqlDBA.php');
    $authkey = md5($authkey . $_SERVER['HTTP_USER_AGENT']);
    $weiboAuthkey = 'i3UtNKGnGZaJwZxM';
    $tuangouAuthkey = 'cHNrV5UHGQdfVOf0';
    $zhuanlanAuthkey = 'i3UtNKGnAZaJwBxM';
    $login = false;
    //var_export(GetCookie('auth');
    if (GetCookie('auth') && empty($username)) {
        $auth = GetCookie('auth');
        $auth = authcode($auth, 'DECODE', $authkey);
        list($password, $uid) = explode("\t", $auth);
        $sSql = " select `uid`,`username`,`password`,`emailstatus` from  " . mysqlDBA::discuz('common_member') . " where `uid`='$uid' limit 0,1 ";
        $value = $db->get_one($sSql);
        if ($value['password'] == $password && !empty($password)) {
            $username = $value['username'];
            setcookie("username_last_login", urlencode($username), time() + 3600 * 24 * 30, "/", COOKIE_DOMAIN);
    ...
    elseif ($username) {
        $sSql = " select `salt`,`password`,`uid`,`secques`,`username` from  " . mysqlDBA::ucenter('members') . " where `username`='$username' limit 0,1 ";
        $value = $db1_3->select($sSql);
        if (empty($value)) {
            $sSql = " select `salt`,`password`,`uid`,`secques`,`username` from  " . mysqlDBA::ucenter('members') . " where `email`='$username' limit 0,1 ";
            $value = $db1_3->select($sSql);
        }
        if (md5(md5($password) . $value[0]['salt']) == $value[0]['password']) {
            $uid = $value[0]['uid'];
            $secques = $value[0]['secques'];
            $username = $value[0]['username'];
            $sSql = " select `uid`,`username`,`password`,`emailstatus` from  " . mysqlDBA::discuz('common_member') . " where `username`='$username' limit 0,1 ";
            $value = $db->get_one($sSql);
            $password = $value['password'];
            Cookie("auth", authcode($password . "\t" . $uid, 'ENCODE', $authkey));
            setcookie("username_last_login", urlencode($username), time() + 3600 * 24 * 30, "/", COOKIE_DOMAIN);

            $sSql2 = " select `password`,`uid` from  car_weibo.weibo_members where `ucuid`='$uid' ";
            $temp = $db1_3->get_one($sSql2);
            $weiboPassword = $temp['password'];
            $weiboUid = $temp['uid'];
            setcookie("JishiGou_NJDPO7_auth", authcode($weiboPassword . "\t" . $weiboUid, 'ENCODE', $weiboAuthkey), time() + 3600 * 24 * 30, "/", COOKIE_DOMAIN);

            $sSql = "SELECT `uid`,`password`,`secques` FROM `tuangou`.`cenwor_system_members` WHERE `ucuid`='$uid'";
            $temp = $db->get_one($sSql);
            setcookie("chezhugroupbuy_cOf4jN_sid", '', time() - 1, "/", COOKIE_DOMAIN);
            setcookie("chezhugroupbuy_cOf4jN_auth", authcodeTuangou($temp['password'] . "\t" . $temp['secques'] . "\t" . $temp['uid'], 'ENCODE', $tuangouAuthkey), time() + 3600 * 24 * 30, "/", COOKIE_DOMAIN);

            $sSql = "SELECT `ucuid`,`password` FROM `car_zhuanlan`.`zl_user` WHERE `ucuid`='$uid' AND `isDel`=0";
            $temp = $db1_3->get_one($sSql);
            if (empty($temp)) {
                $sSql2 = " select `password`,`ucuid`,`iden_domain` from  car_weibo.weibo_members where `ucuid`='$uid' ";
                $temp = $db1_3->get_one($sSql2);
                $value['weibo_domain'] = $temp['iden_domain'];
            }
            setcookie("zhuanlan_NXvAzl_auth", authcode($temp['password'] . "\t" . $temp['ucuid'], 'ENCODE', $zhuanlanAuthkey), time() + 3600 * 24 * 30, "/", COOKIE_DOMAIN);
            //echo authcodeTuangou($temp['password']."\t".$temp['secques']."\t".$temp['uid'],'ENCODE',$tuangouAuthkey);
            unset($value['password']);
            if ($returnArr)
                return $value;
            else
...
function public_verifyLogout() {
    ShiftCookie('auth');
    setcookie("JishiGou_NJDPO7_auth", '', time() - 1, "/", COOKIE_DOMAIN);
    setcookie("chezhugroupbuy_cOf4jN_auth", '', time() - 1, "/", COOKIE_DOMAIN);
    setcookie("chezhugroupbuy_cOf4jN_sid", '', time() - 1, "/", COOKIE_DOMAIN);
    setcookie("zhuanlan_NXvAzl_auth", '', time() - 1, "/", COOKIE_DOMAIN);
}
